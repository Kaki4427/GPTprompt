...
def generate_prompt_18(positive_examples: List[Dict], negative_examples: List[Dict], validation_set, option='sentences'):
...
def complete_prompts_18(prompt: str, num_examples_to_classify) -> Tuple[List[Dict], float]:
...
def classify_validation_set_18(prompt_list, validation_set, ignore_cache=False):
    ...
    response_list = get_model_response(prompt_list, system_prompt, ignore_cache=True)
    search_exp = r"({.*?})\n"
    matches_dict = {}
