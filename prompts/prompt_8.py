...
def get_info_string(sentence_obj):
    ...
    result_string = f"verb: {verb}, direct_object: {direct_object}, preposition: {preposition}, prepositional_object: {prepositional_object}"
    
    return result_string

def generate_prompt_8(positive_examples: List[Dict], negative_examples: List[Dict], validation_set, option='sentences'):
...
def complete_prompts_8(prompt: str, num_examples_to_classify) -> Tuple[List[Dict], float]:
...
def classify_validation_set_8(prompt_list, validation_set, ignore_cache=False):
