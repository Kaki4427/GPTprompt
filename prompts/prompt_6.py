...
def get_substring(sentence_obj):
    pattern = r"({}.*?{})".format(re.escape(sentence_obj['verb']), re.escape(sentence_obj['prepositional_object']))
    match = re.search(pattern, sentence_obj['sentence'])
    if match:
        return match.group(0)
    else:
        raise Exception
    
def one_prompt_string(sentence_obj):
    return f"{sentence_obj['sentence']}, candidate: {get_substring(sentence_obj)}"

def generate_prompt_6(positive_examples: List[Dict], negative_examples: List[Dict], validation_set, option='sentences'):
    positive_prompt = "\n".join([one_prompt_string(datum) for datum in positive_examples])
    negative_prompt = "\n".join([one_prompt_string(datum) for datum in negative_examples])
    validation_prompt = "\n".join([f"{datum['hash']}: {one_prompt_string(datum)}" for datum in validation_set])
...
def complete_prompts_6(prompt: str, num_examples_to_classify) -> Tuple[List[Dict], float]:
...
def classify_validation_set_6(prompt_list, validation_set, ignore_cache=False):
