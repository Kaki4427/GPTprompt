...
def generate_prompt_5(positive_examples: List[Dict], negative_examples: List[Dict], validation_set, option='sentences'):
    positive_prompt = "\n".join([get_substring(datum) for datum in positive_examples])
    negative_prompt = "\n".join([get_substring(datum) for datum in negative_examples])
    validation_prompt = "\n".join([f"{datum['hash']}: {get_substring(datum)}" for datum in validation_set])
...
def complete_prompts_5(prompt: str, num_examples_to_classify) -> Tuple[List[Dict], float]:
...
def classify_validation_set_5(prompt_list, validation_set, ignore_cache=False):
