...
def generate_prompt_1(positive_examples: List[Dict], negative_examples: List[Dict], validation_set, option='sentences'):
    positive_prompt = "\n".join([datum['sentence'] for datum in positive_examples])
    negative_prompt = "\n".join([datum['sentence'] for datum in negative_examples])
    validation_prompt = "\n".join([f"{datum['id']}: {datum['sentence']}" for datum in validation_set])
...
def complete_prompts_1(prompt: str, num_examples_to_classify) -> Tuple[List[Dict], float]:
    instructions = f"Reply with a csv codeblock (wrapped in three backticks), with the headers 'id' and 'label'. Label should be either True or False. Label all {num_examples_to_classify} sentences."
    prompt_with_instructions = f"{prompt}\n\n{instructions}"

    return prompt_with_instructions


def classify_validation_set_1(prompt_list, validation_set, ignore_cache=False):
    system_prompt = ""

    # print("Number of tokens in prompt:", count_tokens(prompt_with_instructions))
    search_exp = r"([0-9]{,4}),(True|False)"
    ...
    for response in response_list:
        ...
        for id, label in matches:
            try:
                ...
                print(id, label)
            matches_dict[int_id] = label.lower() == 'true'

        for datum in validation_set:
            if datum['id'] in matches_dict:
                datum['prediction'] = matches_dict[datum['id']]
