...
NUM_CLASSIFICATION_EXAMPLES = 100
...
def generate_prompt_19(positive_examples: List[Dict], negative_examples: List[Dict], validation_set, option='sentences'):
...
def complete_prompts_19(prompt: str, num_examples_to_classify) -> Tuple[List[Dict], float]:
...
def classify_validation_set_19(prompt_list, validation_set, ignore_cache=False):
    ...
    response_list = get_model_response(prompt_list, system_prompt, ignore_cache=True, model="gpt-4")
    search_exp = r"({.*?})\n"
    matches_dict = {}
    ...
    tokens = sum([count_tokens(prompt)*0.03/1000 + count_tokens(response)*0.06/1000 for prompt, response in zip(prompt_list, response_list)])
